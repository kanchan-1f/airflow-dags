...
dag = DAG(
    '1finance_adoption_dag',
    default_args=default_args,
    description='1finance_adoption data',
...
def main():
    try:
        ...
        query_for_sameday = """
            ...
            AND DATE(install_date) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)       
        """
        # query_for_registration_a = """
        #     with final_flow as ( 
        #     with reg
        #     as (
        #     select distinct customer_ID  , app_version from `MasterDB.1_source_t-1` where date(Registration_date) = DATE_SUB(CURRENT_DATE(), interval 1 DAY)
        #     and Internal_Test_Tagging is null 
        #     )
        #     , flow as (
        ...
        #     select count(distinct customer_ID)  from final_flow where flow != 'B' or flow is null
        # """

        # query_for_registration_b = """
        #     with final_flow as ( 
        #     with reg
        #     as (
        #     select distinct customer_ID  , app_version from `MasterDB.1_source_t-1` where date(Registration_date) = DATE_SUB(CURRENT_DATE(), interval 1 DAY)
        ...
        #     select distinct customer_code ,  'B' as flow from `EosDB.customer_data` where category_id = '134' and parameters = '3')
        #     select  distinct reg.customer_ID ,  app_version ,flow.flow from reg left join flow on reg.customer_ID = flow.customer_ID )
        #     select count(distinct customer_ID) from final_flow where flow = 'B'
        # """
        ...
        query_for_installs_v2_5_2 = """
            ...
            added as (select count(distinct customer_ID)  as b from base where date(install_date) =DATE_SUB(CURRENT_DATE(), interval 1 DAY))
        ...
        select *  from two ) ,
        ins as (
        select count(*) from (select distinct customer_ID ,  av from `MasterDB.1_source_t-1`  m left join (select distinct appsflyer_ID , min(app_version) av from `AppsflyerDB.in_app_events_no_test_v4`
        group by 1) v using(appsflyer_ID)
        ...
        # installs_v2_5_1 = fetch_single_result(query_for_installs_v2_5_1, "Installs v2.5.1")
        # upgrades_v2_5_1 = fetch_single_result(query_for_upgrades_v2_5_1, "Upgrades v2.5.1")
