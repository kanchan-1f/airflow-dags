...
TABLES_TO_TRANSFER = ["cf_assets_projections", "cf_liabilities_projections","customer_profile", "income", "expenses", "liabilities", "insurance", "assets",
    ...
    "cf_projections", "customer_details", "effective_tax_rate","ideal_cf_projections","nps_percent_breakup","customer_inheritance"
]
    ...
    try:
        ...
        with psycopg2.connect(**SRC_DB_CONFIG) as src_conn, psycopg2.connect(**DEST_DB_CONFIG) as dest_conn:
            with src_conn.cursor() as src_cursor, dest_conn.cursor() as dest_cursor:
                ...
                pii_cols = ['mobile_number','email', 'pan_no', 'pan_name']
                src_cursor.execute(f"""
                                        SELECT column_name 
                                        FROM information_schema.columns 
                                    ...
                                    """, (tuple(pii_cols),))
                cols_to_keep = [row[0] for row in src_cursor.fetchall()]
                # col_list = ', '.join(cols_to_keep)
                # select_expr = ', '.join([f"NULLIF({col}, '') AS {col}" for col in cols_to_keep])
                select_expr = ', '.join([f"NULLIF({col}::text, '') AS {col}" for col in cols_to_keep])

                # src_cursor.copy_expert(f"COPY {table_name} TO STDOUT WITH CSV HEADER", mem_buffer)
                src_cursor.copy_expert(f"COPY (SELECT {select_expr} FROM {table_name}) TO STDOUT WITH CSV HEADER", mem_buffer)
                # src_cursor.copy_expert(f"""
                #     COPY (
                #         SELECT 
                #             {', '.join([
                #                 f"NULLIF({col}, '') AS {col}" 
        ...
        raise f'Error while transferring {table_name}: {e}'
